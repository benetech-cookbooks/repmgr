...
node.default[:repmgr][:config][:conninfo] = "host=#{node[:repmgr][:addressing][:self]} user=#{node[:repmgr][:replication][:user]} dbname=#{node[:repmgr][:replication][:database]} sslmode=#{node[:repmgr][:replication][:sslmode]}"
